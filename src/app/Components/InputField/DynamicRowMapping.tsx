...
const DynamicRowMapping: React.FC<DynamicRowMappingProps> = ({
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const data = await deleteSocial(id);
      if (data?.success) {
  ...
  return (
    <div className=" w-full">
      <div className=" mb-2 border-b pb-2   flex w-full">
        <button
          onClick={addSocialLink}
          className="inline-flex items-center  text-blue-500 rounded-full border border-dashed border-gray-200 bg-white px-2 py-1.5 text-xs font-medium  hover:bg-gray-50 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
        >
          <svg
